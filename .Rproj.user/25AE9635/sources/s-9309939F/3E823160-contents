...
#' @importFrom data.table year month .SD
...
agg_to_monthly <- function(dt, date_name = NULL){
  ...
  if(is.null(date_name)){
    date_ind  <- which(grepl("date", colnames(dt), ignore.case = T) || grepl("time", colnames(dt), ignore.case = T))
  }else{
    date_ind  <- which(colnames(dt) == date_name)
  }

  if(length(date_ind)==0){
    stop("Date column not found in dt")
  }else if(length(date_ind)>1){
    stop("More than one column contain date")
  }

  var_names         <- colnames(dt)[-date_ind]
  colnames(dt)[date_ind] <- "DATE"

  if(median(diff.Date(as.Date(dt$DATE))) >= 28){
    stop("Data is not high frequency (less than monthly)")
  }

  dt_out   <- dt[,  lapply(.SD, mean_if_avail), by = .(year = data.table::year(dt$DATE), month = data.table::month(dt$DATE)), .SDcols = var_names]
